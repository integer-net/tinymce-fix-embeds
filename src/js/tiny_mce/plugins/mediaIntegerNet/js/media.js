(function() {
	...
	window.Media = {
		...
		insert : function() {
			...
			editor.selection.setNode(editor.plugins.mediaIntegerNet.dataToImg(this.data));
			tinyMCEPopup.close();
		},

		preview : function() {
			get('prev').innerHTML = this.editor.plugins.mediaIntegerNet.dataToHtml(this.data, true);
		},

		moveStates : function(to_form, field) {
			var data = this.data, editor = this.editor,
				mediaPlugin = editor.plugins.mediaIntegerNet, ext, src, typeInfo, defaultStates, src;
		...
		formToData : function(field) {
			...
			if (field == 'source') {
				...
				setVal('source', this.editor.plugins.mediaIntegerNet.dataToHtml(this.data));
				this.panel = 'source';
			} else {
				if (this.panel == 'source') {
					this.data = clone(this.editor.plugins.mediaIntegerNet.htmlToData(getVal('source')));
					this.dataToForm();
					this.panel = '';
				}
